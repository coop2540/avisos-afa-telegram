...
- `repo-publishing`: preparación y publicación del repositorio del proyecto — privado primero, público en GitHub después — con licencia, README, ausencia de secretos y documentación pública del propio flujo.
...
- Infra: un VPS pequeño (1 vCPU / ~1 GB) o un servidor propio en fase inicial; secretos fuera del repo (variable de entorno o gestor de secretos); sin Postgres/Redis.
- Nextcloud (u otro almacenamiento propio) solo en fases B/C para espejo opcional y alojo de la doc de transparencia.
- Repositorios: instancia propia (privada) como primer destino; **GitHub** público tras aprobación. Llicència oberta (per defecte MIT, revisable).
