...
- **Tria:** publicar el dia anterior a les 19:00 amb el títol \"Demà dinem …\". Lògica paral·lela a `should_post_weekly`: calcular la data objectiu (demà, o el proper dia lectiu si demà no ho és — vegeu D5), i publicar si `ara >= slot` i `menu_posted[variant] != data_objectiu`. **Actualització post-desplegament (24/09/2026):** el \"slot-aware sleep\" s'ha implementat al scheduler (`next_interval_minutes(menu=…, tz=…)`) perquè l'interval del cicle s'acosti a l'slot de les 19:00 (o reintentarà als 10 min si l'slot ja ha passat i encara no s'ha resolt); abans l'arribada depenia de la deriva del sondeig base de 60 min (19:00→~20:00).
...
- **Slot-aware sleep (implementat el 24/09/2026, fora de l'alcance original):** afegit a `next_interval_minutes` com a variante que D3 preveia. No canvia el contracte: només limita l'interval per arribar a l'slot diari (`next_menu_deadline`); amb el menú deshabilitat o sense `tz` el comportament és l'anterior. A més, `run_forever` recarrega `state` des de disc abans de cada cicle (`State.reload`) perquè un procés extern (`--once`) no provoqui duplicats.
...
- **[Deriva 19:00→20:00 pel sondeig base de 60 min]** → **RESOLT (24/09/2026):** slot-aware sleep al scheduler (limita l'interval fins a l'slot de les 19:00; reintent als 10 min si l'slot ha passat sense resoldre's). Deriva residual: pocs minuts (temps del cicle), no hores.
...
- **[Escriptor extern concurrent (`--once` mentre el bucle és dins `run_cycle`)]** → el `State.reload` abans de cada cicle tanca la finestra de la dormida (minuts), però no la del cicle actiu (segons): sense lock de fitxer, qui desa l'últim guanya i podria perdre's un canvi de l'altre procés. Operativament: no llançar `--once` manualment quan el servei estigui viu; la dedup per data del menú i el pin fan que un perdut es resolgui al cicle següent.
